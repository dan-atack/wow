...
function ReflexCheck({ move, combo, numPrevMoves }) {
    ...
    React.useEffect(() => {
        // If you've failed, ensure the string is exactly zero (it can be imprecise otherwise):
        if (failStatus) {
        ...
        if (move.combos[combo].length === currentKey && !failStatus) setSuccessStatus(true);
    ...
    return (
        <Wrapper failure={failStatus} success={successStatus}>
            ...
            <h4>STATUS: {failStatus ? 'DEAD': successStatus ? 'SUCCESS!!!' : 'ONGOING'}</h4>
            <h4>KEYS:</h4>
